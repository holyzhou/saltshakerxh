...
class SaltAPI(object):
    __token_id = ''
    def __init__(self):
        self.__url = 'https://127.0.0.1:8000'
        self.__user = 'saltapi'
        self.__password = 'pubinfo!@#'
    def token_id(self):
    ...
    def remote_noarg_execution(self,tgt,fun):
        ...
        params = {'client': 'local', 'tgt': tgt, 'fun': fun,'expr_form':'pcre'}
        obj = urllib.urlencode(params)
        self.token_id()
        content = self.postRequest(obj)
	...
	if arg in ['disk.usage','network.interfaces','grains.items','test.ping','state.running','status.meminfo','status.uptime','service.get_all']:
       	     params = {'client': 'local', 'tgt': tgt, 'fun': arg, 'expr_form': 'list'}
	elif arg == 'jobs.lookup_jid':
	     params = {'client': 'runner', 'tgt': tgt, 'fun': arg, 'jid': arg1, 'expr_form': 'list'}
